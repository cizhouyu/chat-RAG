...
public class Milvus {
    ...
    public Milvus(String collectionName) {
        // 1. 连接 Milvus 服务
...
//        String collectionName = "demo_collection418";
    ...
    public void build_dataset (List<JsonProcessor.DialogPair> dialogPairs) throws Exception{  // 如果不加 throws Exception，在调用qsentence2Vector时会报错。
        ...
        for (JsonProcessor.DialogPair pair : dialogPairs) {
            ...
            this.milvusClient.flush(
                    FlushParam.newBuilder()
                            .withCollectionNames(Collections.singletonList(this.collectionName))
                            .build()
            );
        }
        ...
        this.milvusClient.createIndex(createIndexParam);
        ...
        System.out.println("集合创建成功，字段: id, context, reply, embedding");
    }

    public void search (String sentence) throws Exception{
        // 5. 用新的句子“哈啰，今天天气如何？”生成查询向量
        List<List<Float>> searchVectors = qsentence2Vector(sentence);
